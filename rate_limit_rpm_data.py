...
def validate_rate_limited_dataset(dset):
    for i in range(1, len(dset)):
        if abs(dset[i] - dset[i-1]) > MAX_dy:
...
if __name__=="__main__":
    ...
    for file in files:
        ...
        result = rate_limit(y_arr)

        if not validate_rate_limited_dataset(dset=result):
        ...
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(f"results/{file.split(".")[0]}.png", dpi=300, bbox_inches="tight")
        #plt.show()

        f = open(f"results/{file.split(".")[0]}_rate_limited.csv", "w")
